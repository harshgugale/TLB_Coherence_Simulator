...
public:
    ...
    Request(uint64_t addr,
    		kind type,
			uint64_t tid,
			bool is_large,
			unsigned int core_id,
			std::function<void(std::shared_ptr<Request>)> callback = nullptr,
			bool is_memory_acc = true) :
    m_addr(addr),
    m_type(type),
    m_core_id(core_id),
    m_callback(callback),
    ...
    friend std::ostream& operator << (std::ostream &out, const Request &r)
    {
    	out << "is_memory_acc : " << r.m_is_memory_acc << std::hex <<
				...
				", core: " << r.m_core_id <<
				", is_agnostic:" << r.m_is_core_agnostic <<
				", is_guest_shootdown " << r.is_guest_shootdown <<
				", is_migration_shootdown " << r.is_migration_shootdown << std::endl;
        return out;
    }
